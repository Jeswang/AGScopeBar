...
// These font macros added by Seth Willits
...
// Seth Willits -- default is no, behavior changed
#define SCOPE_BAR_HIDE_POPUP_BG			NO	// whether the bezel background of an NSPopUpButton is hidden when none of its menu-items are selected.
...
- (NSButton *)buttonForItem:(NSString *)identifier inGroup:(NSInteger)groupNumber 
				  withTitle:(NSString *)title image:(NSImage *)image menu:(NSMenu *)menu; // creates a new NSButton
- (NSMenuItem *)menuItemForItem:(NSString *)identifier inGroup:(NSInteger)groupNumber 
					  withTitle:(NSString *)title image:(NSImage *)image menu:(NSMenu *)itemMenu; // creates a new NSMenuitem
		...
		if (numGroups > 0) {
			...
			for (int groupNum = 0; groupNum < numGroups; groupNum++) {
				...
				if (groupLabel && [groupLabel length] > 0) {
					...
					[labelField setFont:SCOPE_BAR_FONT];
				...
				for (NSString *itemID in identifiers) {
					...
					if ([delegate respondsToSelector:@selector(scopeBar:menuForItem:inGroup:)]) {
						menu = [delegate scopeBar:self menuForItem:itemID inGroup:groupNum];
					}
					
					NSButton *button = [self buttonForItem:itemID inGroup:groupNum withTitle:title image:image menu:menu];
...
// Added by Seth Willits
- (BOOL)isItemSelected:(NSString *)identifier inGroup:(NSInteger)groupNumber;
{
	return [[_selectedItems objectAtIndex:groupNumber] containsObject:identifier];
}
...
- (void)setEnabled:(BOOL)enabled forItem:(NSString *)identifier inGroup:(NSInteger)groupNumber;
{
...
- (BOOL)isItemEnabled:(NSString *)identifier inGroup:(NSInteger)groupNumber;
{
	NSButton * button = [self getButtonForItem:identifier inGroup:groupNumber];
...
- (void)setDelegate:(id<MGScopeBarDelegate>)newDelegate
{
...
- (id<MGScopeBarDelegate>)delegate
{
	return delegate;
}
...
#pragma mark  
#pragma mark  
@implementation MGScopeBar (MGPrivateMethods)
...
- (NSButton *)buttonForItem:(NSString *)identifier inGroup:(NSInteger)groupNumber 
				  withTitle:(NSString *)title image:(NSImage *)image menu:(NSMenu *)menu
{
	...
	// --------------------------------------------
	if (menu) {
		BOOL pullsDown = (title != nil);
		MGRecessedPopUpButtonCell * cell = [[[MGRecessedPopUpButtonCell alloc] initTextCell:@"" pullsDown:pullsDown] autorelease];
		NSMenuItem * titleItem = [[[NSMenuItem alloc] init] autorelease];
		
		button = [[NSPopUpButton alloc] initWithFrame:ctrlRect pullsDown:pullsDown];
		[button setCell:cell];
		[cell setMenu:menu];
		...
		[menu setFont:SCOPE_BAR_MENUITEM_FONT];
		
		
		// Standard Button
		// --------------------------------------------
	} else {
		button = [[NSButton alloc] initWithFrame:ctrlRect];
	}
...
- (NSMenuItem *)menuItemForItem:(NSString *)identifier inGroup:(NSInteger)groupNumber 
					  withTitle:(NSString *)title image:(NSImage *)image menu:(NSMenu *)itemMenu
{
	...
	// Seth Willits
	[[[[itemMenu itemArray] lastObject] parentItem] setSubmenu:nil]; // workaround for some bug
...
- (NSPopUpButton *)popupButtonForGroup:(NSDictionary *)group
{
	...
	[[popup menu] setFont:SCOPE_BAR_MENUITEM_FONT]; // Seth Willits
	[popup setFont:SCOPE_BAR_FONT];
	...
	[popup setShowsBorderOnlyWhileMouseInside:YES];
	[[popup cell] setAltersStateOfSelectedItem:NO];
	[[popup cell] setArrowPosition:NSPopUpArrowAtBottom];
	
	
	
	// Seth Willits -- mmm, Apple doesn't do this, and it feels weird anyway.
